...
#define NR_MAX_CHAR_INROLL      8  
#define NR_MAX_CHAR_INNAME      16
#define NR_MAX_CHAR_INATTND     8 
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    char szRollBuffer[BUFFER_SIZE]; 
    char szAttendanceBuffer[BUFFER_SIZE]; 
    int i; 
    ...
    {   
        ...
        case WM_PAINT: 
            ...
            for(i = 0; i < NR_STUDENTS; ++i)
            {
                ...
                TextOut(hdc, 0, i*cyChar, szRollBuffer, lstrlen(szRollBuffer)); 
                TextOut(
                        hdc, 
                        NR_MAX_CHAR_INROLL * cxChar, 
                        i * cyChar, 
                        strec[i].szStudentName, 
                        lstrlen(strec[i].szStudentName)
                    ); 
                TextOut(
                        hdc, 
                        NR_MAX_CHAR_INROLL * cxChar + NR_MAX_CHAR_INNAME * cxCaps, 
                        i * cyChar, 
                        szAttendanceBuffer, 
                ...
                TextOut(
                    hdc, 
                    (NR_MAX_CHAR_INROLL + NR_MAX_CHAR_INATTND) * cxChar + NR_MAX_CHAR_INNAME * cxCaps, 
                    i * cyChar, 
                    strec[i].szCity, 
